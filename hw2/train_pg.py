...
def train_PG(exp_name='',
    ...
    for itr in range(n_iter):
        ...
        q_n = []
        for path in paths:
            times = np.arange(pathlength(path))
            discount = gamma**times
            rewards = path['reward'] * discount

            if not reward_to_go:
                q_n.append(np.ones(pathlength(path)) * np.sum(rewards))
            else:
                q_n.append(np.cumsum(rewards[::-1])[::-1] / discount)
        ...
        # YOUR_CODE_HERE
        new_loss, _ = sess.run([loss, update_op],
